...
\begin{document}
    ...
    \begin{lstlisting}[style=Matlab-editor, caption={MATLAB script for OTA computation aggregation using the \gls{rrc} filter.}, label={lst:ideal_ota_rc}]
    ...
    \begin{figure}[htbp]
        ...
        \begin{subfigure}{.45\linewidth}
            \begin{tikzpicture}
                ...
                \end{axis}
            \end{tikzpicture}
            \caption{\textsf{RC-Filtered}}
            \label{plt:rcOTAmse}
        \end{subfigure}
        \caption{Comparison of MSE performance between the ideal baseline and the Raised Cosine filtered scenario.}
    ...
    \begin{figure}[htbp]
        ...
        \begin{subfigure}{0.45\linewidth}
            \includegraphics[width=\linewidth, height=0.8\linewidth]{data/fusion_eye_90sps.eps}
        ...
        \begin{subfigure}{0.45\linewidth}
            \begin{tikzpicture}
                \begin{axis}[
                    ...
                    xlabel={In-Phase (I)},
                    ylabel={Quadrature (Q)},
                    ymin=-1, ymax=1, grid=major,% Lock the Y-axis since Q is always 0
                    ...
                    ] table[x=I_Data, y=Q_Data] {data/fc_constellation_90sps.txt};
                \end{axis}
            \end{tikzpicture}
        ...
        \caption{Signal integrity analysis at the Fusion Center. Left: Eye diagram illustrating Inter-Symbol Interference bounds. Right: Native scatter plot of the real-valued symbol constellation.}
    ...
    Raised Cosine pulse shaping is presented in Figure~\ref{fig:idealOTA_simulink_rrc}. This architecture 
    ...
    \begin{figure}[htbp]
        ...
        \begin{subfigure}{.45\linewidth}
            \begin{tikzpicture}
                \begin{axis}[
                    title={\textsf{Constellation Diagram without a channel}},
                    xlabel={In-phase Amplitude}, ylabel={Quadrature Amplitude},
                    ymin=-0.5, ymax=1.5, grid=major,
                    width=\linewidth, height=0.8\linewidth,
                    legend pos=north east,
                    mark=none, thick
                ]
                    \addplot[
                        ...
                        color=blue,
                        opacity=0.4
                    ] table[x=ConstellationDiagram, y expr=0] {data/simulink_fc_constellation.txt};
            
                \end{axis}
            \end{tikzpicture}
            \caption{Constellation Diagram of the ideal OTA computation network over an \gls{awgn} channel.}
            \label{fig:simulink_Constellation_no_ch}
        \end{subfigure}%
        \hfill
        \begin{subfigure}{.45\linewidth}
            \begin{tikzpicture}
                \begin{axis}[
                    title={\textsf{Constellation Diagram with a channel}},
                    xlabel={In-phase Amplitude}, ylabel={Quadrature Amplitude},
                    ymin=-0.5, ymax=1.5, grid=major,
                    width=\linewidth, height=0.8\linewidth,
                    legend pos=north east,
                    ...
                    \addplot[
                        ...
                        color=blue,
                        opacity=0.4
                    ] table[x=ConstellationDiagramCh, y expr=0] {data/simulink_fc_constellation.txt};

                \end{axis}
            \end{tikzpicture}
            \caption{Constellation Diagram of the Raised Cosine filtered OTA computation network over an \gls{awgn} channel.}
            \label{fig:simulink_Constellation_ch}
        \end{subfigure}
        \caption{Simulink Constellation Diagrams}
        \label{fig:simulink_constellation_comparison}
    \end{figure}

    \begin{figure}[htpb]
        \centering
        \includegraphics[width=\linewidth]{flowgraphs/idealOTA_simulink_rrc.pdf}        
        \caption{Simulink block diagram representing the ideal OTA computation network over an \gls{awgn} channel.}
        \label{fig:idealOTA_simulink_rrc} 
    \end{figure}
    ...
    % Consider adding a figure from the Simulink simulation showing the MSE over time or SNR to visually 
    % reinforce the performance analysis.

    The constellation diagrams provide a clear visual confirmation of the role played by
    pulse shaping in the received signal. The plot corresponding to the fusion center
    observation after the matched RRC filter shows a more compact and well-defined symbol
    distribution, indicating that the filtering stage has improved temporal alignment and
    reduced the impact of inter-symbol interference. In contrast, the constellation 
    associated with the theoretical receiver model without pulse shaping appears more
    dispersed and less structured, reflecting the absence of the smoothing and
    energy-concentrating effect provided by the matched filter. This comparison highlights
    that the matched RRC filter contributes not only to spectral shaping, but also to
    preserving symbol integrity and improving the reliability of the recovered OTA
    estimate.
    ...
    \begin{figure}[htbp]
        ...
        \begin{subfigure}{.45\linewidth}
            \begin{tikzpicture}
                \begin{axis}[
                    title={\textsf{Constellation Diagram Matched Filter}},
                    xlabel={In-phase Amplitude}, ylabel={Quadrature Amplitude},
                    ymin=-0.5, ymax=1.5, grid=major,
                    width=\linewidth, height=0.8\linewidth,
                    legend pos=north east,
                    ...
                    \addplot[
                        ...
                        color=blue,
                        opacity=0.4
                    ] table[x=ConstellationDiagramMatched_Layer1, y expr=0] {data/simulink_fc_constellation_matched_rrc.txt};
                    ...
                    \addplot[
                        ...
                        color=red,
                        opacity=0.4
                    ] table[x=ConstellationDiagramMatched_Layer2, y expr=0] {data/simulink_fc_constellation_matched_rrc.txt};
                    \addlegendentry{Layer 2}
            
                \end{axis}
            \end{tikzpicture}
            \caption{Constellation Diagram of the ideal OTA computation network over an \gls{awgn} channel.}
            \label{fig:simulink_Constellation_ideal}
        \end{subfigure}%
        \hfill
        \begin{subfigure}{.45\linewidth}
            \begin{tikzpicture}
                \begin{axis}[
                    title={\textsf{Constellation Diagram No filter}},
                    xlabel={In-phase Amplitude}, ylabel={Quadrature Amplitude},
                    ymin=-0.5, ymax=1.5, grid=major,
                    width=\linewidth, height=0.8\linewidth,
                    legend pos=north east,
                    ...
                    \addplot[
                        ...
                        color=blue,
                        opacity=0.4
                    ] table[x=ConstellationDiagramNoFilt_Layer1, y expr=0] {data/simulink_fc_constellation_rrc.txt};
                    \addlegendentry{Layer 1}
                    ...
                    \addplot[
                        ...
                        color=red,
                        opacity=0.4
                    ] table[x=ConstellationDiagramNoFilt_Layer2, y expr=0] {data/simulink_fc_constellation_rrc.txt};
                    \addlegendentry{Layer 2}

                \end{axis}
            \end{tikzpicture}
            \caption{Constellation Diagram of the Raised Cosine filtered OTA computation network over an \gls{awgn} channel.}
            \label{fig:simulink_Constellation_rc}
        \end{subfigure}
        \caption{Simulink Constellation Diagrams}
        \label{fig:simulink_constellation}
    \end{figure}

    The eye diagrams further illustrate the impact of pulse shaping on the received
    signal. The eye diagram corresponding to the fusion center observation after the
    matched RRC filter shows a more open and well-defined eye pattern, indicating that the
    filtering stage has improved temporal alignment and reduced inter-symbol interference.
    In contrast, the eye diagram associated with the theoretical receiver model without
    pulse shaping appears more closed and less structured, reflecting the absence of the
    smoothing and energy-concentrating effect provided by the matched filter. This
    comparison highlights that the matched RRC filter contributes not only to spectral
    shaping, but also to preserving symbol integrity and improving the reliability of the
    recovered OTA estimate.

    \begin{luacode*}
    
        function process_dual_eye(input_file, output_file, sps, max_traces)
            
            local samples_per_trace = 2 * sps
            local infile = io.open(input_file, "r")
            local outfile = io.open(output_file, "w")

            if not infile then
                tex.print("% Error: Could not open " .. input_file)
                return
            end

            -- Write the unified headers for pgfplots
            outfile:write("Time\tLayer1\tLayer2\n")
            
            -- Discard the original header
            ...
            for line in infile:lines() do
                if trace_count >= max_traces then break end

                -- Extract BOTH data columns safely
                local layer1, layer2 = string.match(line, "^%s*([^%s]+)%s+([^%s]+)")
                
                if layer1 and layer2 then
                    local time_val = sample_count / sps
                    
                    -- Output Time, Layer 1, and Layer 2
                    outfile:write(string.format("%f\t%s\t%s\n", time_val, layer1, layer2))
                    
                    sample_count = sample_count + 1

                    -- Insert the blank line for pgfplots to segment the traces
                    if sample_count >= samples_per_trace then
                        outfile:write("\n") 
                        sample_count = 0
        ...
        -- Process both files before rendering the figures.
        -- (Adjust '100' to match the actual SPS used in your Simulink export)
        process_dual_eye("data/simulink_fc_eye.txt", "data/processed_nofilt.txt", 8, 150)
        process_dual_eye("data/simulink_fc_eye_matched_rrc.txt", "data/processed_matched.txt", 8, 150)
    \end{luacode*}
            ...
            \begin{tikzpicture}
                \begin{axis}[
                    ...
                    every axis plot/.append style={
                        mark=none,
                        line width=0.4pt,
                        opacity=0.2 % Slightly higher opacity since layers compete for contrast
                    },
                    legend pos=south east,
                    legend style={fill opacity=0.8, draw opacity=1, text opacity=1}
                ]
                    % Layer 1 (Blue)
                    \addplot[color=blue] table[x=Time, y=Layer1] {data/processed_nofilt.txt};
                    \addlegendentry{Layer 1}
                    
                    % Layer 2 (Red)
                    \addplot[color=red] table[x=Time, y=Layer2] {data/processed_nofilt.txt};
                    \addlegendentry{Layer 2}
                \end{axis}
            \end{tikzpicture}
            \caption{Unfiltered signal.}
            \label{fig:eye_nofilt}
        \end{subfigure}% <-- Crucial to prevent wrapping
        \hfill
        \begin{subfigure}{0.48\linewidth}
            \begin{tikzpicture}
                \begin{axis}[
                    title={Fusion Center (Matched Filter)},
                    xlabel={Time ($T_{sym}$)},
                    ylabel={Amplitude},
                    xmin=0, xmax=2,
                    grid=major,
                    ...
                    % Layer 1 (Blue)
                    \addplot[color=blue] table[x=Time, y=Layer1] {data/processed_matched.txt};
                    \addlegendentry{Layer 1}
                    
                    % Layer 2 (Red)
                    \addplot[color=red] table[x=Time, y=Layer2] {data/processed_matched.txt};
                    \addlegendentry{Layer 2}
                \end{axis}
            \end{tikzpicture}
            \caption{Signal after RRC filtering.}
            \label{fig:eye_matched}
        \end{subfigure}

        \caption{Comparative eye diagrams demonstrating the signal reconstruction at the fusion center.}
        \label{fig:eye_comparison}
    \end{figure}

    % \begin{figure}[htpb]
    %     \centering
    %     \begin{tabular}{ccc}
    %         \videogriditem{0.31\linewidth}{video/E_Zmax1_027e2MHz.mkv}{E_Zmax1_027e2MHz.png} &
    %         \videogriditem{0.31\linewidth}{video/E_Zmax1_077e2MHz.mkv}{E_Zmax1_077e2MHz.png} &
    %         \videogriditem{0.31\linewidth}{video/E_Zmax1_127e2MHz.mkv}{E_Zmax1_127e2MHz.png} \\[1ex]
    %         
    %         \videogriditem{0.31\linewidth}{video/E_Zmax2_027e2MHz.mkv}{E_Zmax2_027e2MHz.png} &
    %         \videogriditem{0.31\linewidth}{video/E_Zmax2_077e2MHz.mkv}{E_Zmax2_077e2MHz.png} &
    %         \videogriditem{0.31\linewidth}{video/E_Zmax2_127e2MHz.mkv}{E_Zmax2_127e2MHz.png} \\
    %     \end{tabular}
    ...
    % write a paragraph about the flowgraph and the ideal scenario that cannot be implemente in real hardware

    

    % write a paragraph about the flowgraph and the ideal scenario that cannot be implemente in real hardware with the matched RRC filter

    \begin{figure}[htpb]
        \centering
        \includegraphics[width=\linewidth]{flowgraphs/idealOTA_grc_rrc.pdf}

        \caption{GNU Radio flowgraph for the ideal, Nyquist-compliant scenario.}
        \label{fig:grc_ideal_rrc}
    \end{figure}
    ...
    \begin{figure}[htpb]
        ...
        \includegraphics[width=\linewidth]{img/idealOTA_rrc.png}
